...
rock.addEventListener('click', () => {
    let playerMove = rock.getAttribute('id');;
    let endGame = playRound(playerMove);

    if (endGame[1] === true) {
        counter[0]++;
    } else if (endGame[1] === 'tie'){
        counter[0] += 0
    } else {
        counter[1]++;
    }

    gameStatus.textContent = endGame[0];
    playerScore.textContent = `You: ${counter[0]}`
    machineScore.textContent = `Computer: ${counter[1]}`
    return;
})

paper.addEventListener('click', () => {
    let playerMove = paper.getAttribute('id');;
    let endGame = playRound(playerMove);
...
scissors.addEventListener('click', () => {
    let playerMove = scissors.getAttribute('id');;
    let endGame = playRound(playerMove);
